...
```
Q3. The choropleth map below highlights the New York City districts with the highest fractions of dead trees. Districts most affected by poor tree health are concentrated on Staten Island, as well as parts of Queens and Brooklyn, where over 14% of its trees are dead.
...
baruch_point <- new_st_point(lat =  40.7394, lon= -73.9833)

#Compute distance from each tree to Baruch
trees_with_district <- trees_with_district |>
  mutate(distance_to_baruch = st_distance(geometry, baruch_point))

#Determine the closest tree
closest_tree <- trees_with_district |>
  arrange(distance_to_baruch) |>
  slice(1)  # top 1 closest tree

closest_tree$genusspecies # species of that tree

line_to_tree <- st_sfc(
  st_cast(st_union(c(baruch_point, closest_tree$geometry)), "LINESTRING"),
  crs = st_crs(baruch_point)
)

# Plot the map
ggplot() +
  # NYC Council Districts
  geom_sf(data = council_districts, fill = "gray95", color = "black") +
  # All trees (light green, low opacity)
  geom_sf(data = trees_with_district, color = "green", alpha = 0.01, size = 0.05) +
  # Closest tree (red)
  geom_sf(data = closest_tree, color = "red", size = 3) +
  # Baruch campus (blue)
  geom_sf(data = baruch_point, color = "blue", size = 3, shape = 18) +
  # Line connecting Baruch and the closest tree
  geom_sf(data = line_to_tree, color = "purple", size = 1, linetype = "dashed") +
  labs(
    title = "Closest Tree to Baruch College",
    subtitle = paste0("Species: ", closest_tree$genusspecies),
    caption = "Data: NYC Open Data and NYC Planning"
  ) +
  theme_minimal() +
  theme(
